...
import type { LoginProps, LoginStyles, LoginProvider, LoginSocial } from "./LoginForm.types";
...
const LoginForm = ({
  ...
  useEffect(() => {
    setStyle({
      ...
      providerButton: {
        withText: cn("flex-1 w-full center p-6", styles?.providerButton?.withText),
        withoutText: cn("w-fit h-fit rounded-full p-3", styles?.providerButton?.withoutText)
      },
      text: cn("", styles?.text || ""),
      ...
      <section className={`${style.container} ${style.text}`}>
        <div className="full flex flex-col gap-2 overflow-y-auto">
          {authOptions.social?.map((options: LoginSocial) => (
            ...
            >
              <img
                src={providersTOSvg[options.provider as LoginProvider]}
