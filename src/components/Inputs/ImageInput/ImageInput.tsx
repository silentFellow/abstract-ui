...
const ImageInput = forwardRef<HTMLInputElement, ImageInputProps>(({
  ...
  useEffect(() => {
    setStyle({
      ...
      input: cn(`cursor-pointer border-none bg-transparent outline-none file:text-[rgb(33,33,33)] dark:file:text-[#f5f5f5] ${!showInput && "hidden"}`, styles?.input || ""),
      image: cn("rounded-full full object-cover", styles?.image || ""),
      imageContainer: cn("flex h-24 w-24 shadow-xl items-center justify-center rounded-full", styles?.imageContainer || "")
    });
  }, [styles, showInput]);
