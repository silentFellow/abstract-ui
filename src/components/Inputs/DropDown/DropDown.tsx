...
const DropDown = ({
  ...
  useEffect(() => {
    setStyle({
      button: cn("bg-[#f5f5f5] dark:bg-[rgb(33,33,33)] full rounded-md no-focus", styles?.button || ""),
      dropdown: cn("mt-1 w-64 flex flex-col px-3 bg-[#f5f5f5] dark:bg-[rgb(33,33,33)] shadow-xl", styles?.dropdown || ""),
  ...
  const filteredOptions = options.filter((option: DropDownOptions) => option.label.includes(filter));
